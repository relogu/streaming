...
class Array:
    ...
    def __getitem__(self, at: Union[int, slice, list[int], NDArray[np.int64]]) -> Any:
        ...
        if isinstance(at, (int, np.integer)):
            if -self.size <= at < 0:
                at += self.size
            return self.get_item(at)  # type: ignore[reportGeneralTypeIssues]
            ...
            for idx in self._each_slice_index(at):
                ...
                items.append(item)
            return items
        elif isinstance(at, list):  # type: ignore[reportUnnecessaryIsInstance]
            items = []
            for sub in at:
            ...
            items = []
            for sub in at:  # type: ignore[reportGeneralTypeIssues]
                item = self.__getitem__(sub)
                items.append(item)
            return items
        else:
